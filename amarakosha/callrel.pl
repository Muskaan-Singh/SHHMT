...
$pid = $ARGV[4];
      ...
      if($relation eq "tree"){
         system ("mkdir -p /tmp/SKT_TEMP/AMARA_DOT");
         system("$myPATH/amarakosha/get_all_rel.pl $word_wx $out_encoding | /usr/bin/dot -Tjpg -o /tmp/SKT_TEMP/AMARA_DOT/graph$pid.jpg");
         print"<center><img src=\"http://localhost/scl/amarakosha/DOT/graph$pid.jpg\" \/></center><br>";
      }else {
         my $str =  "$myPATH/amarakosha/relations.pl $dbm_fl_nm{$relation} '$headers{$relation}' $word_wx $out_encoding $pid| /home/muskaan/scl/converters/";
